...
function submit(){
    Form.post(route('Admin.Topik.store'), {
        preserveState:true,
        onSuccess:()=>{
...
function submitEdit(){
    EditForm.put(route('Admin.Topik.update'), {
        preserveState:true,
        onSuccess:()=>{
          CloseModal()
        }
...
function submitDelete(){
    DeleteForm.delete(route('Admin.Topik.destroy'), {
        preserveState:true,
        preserveScroll:true,
        onSuccess:()=>{
          CloseModal()
    ...
    <AuthenticatedLayout>
        ...
        <div class="bg-white ">
            ...
            <DrawerModal :modalshow="ModalShow">
                <template #title>
                    <span class="text-white">Tambah Topik</span>
                </template>
                <template #close>
                ...
                <template #content>
                    <form @submit.prevent="submit" class="space-y-4">
                        <div class="flex flex-col gap-2">
                            <InputLabel for="nama" class="!text-base">Nama Mata Pelajaran</InputLabel>
            ...
            <DrawerModal :modalshow="EditModal">
                <template #title>
                    <span class="text-white">Tambah Mata Pelajaran</span>
                </template>
                <template #close>
                    <div class="" @click="CloseModal">
                        <svg aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"
            ...
            <CardTable :datalink="topik" :url="'Admin.Topik.index'" :filterdata="false">
                <template #title>
                    Data Mata Pelajaran
                </template>
                <template #add>
                ...
                <template #body>
                    <tr v-for="(item, index) in topik.data" :key="item.id" :index="index">
                        <th class="border-t-0 px-1.5 md:px-3 align-middle border border-black whitespace-nowrap p-4">
                            {{ (topik.current_page - 1) * topik.per_page + index + 1 }}
                        </th>
